...
** 日期转换（PropertyEditor/Convertor/Formater）
...
Spring 3 之后增加了一个 @ControllerAdvice 注解，作用在类上。
它会将类中所有的 @ExceptionHandler/@InitBinder/@ModelAttribute 等全局化。

所以，一般情况下，我们可以将所有 @ExceptionHandler 方法统一放置在一个加了 @ControllerAdvice 的控制器里，实现全局异常的目的。
...
所有的 mapping，按照 RESTful 风格，我们可以设计为：
| 功能         | URL                | 请求的方法 |
