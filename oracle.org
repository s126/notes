...
** 变量类型
1. 标量类型，就是在 sql 中用到的数据类型，比如 varchar2, number, date, blob, raw 等。
   #+BEGIN_SRC sql
   -- 这是最常用的数值类型的定义
   declare
     a number;
...
exception
  when others then
    dbms_output.put_line('haha，你也会有今天。');
end;
...
#+BEGIN_SRC sql
-- 里面有两种异常，一种是字母为零的异常，另一种呢，是没有查询到数据的异常。
-- 这两种都是 Oracle 已经自定义好的异常，我们可以直接使用。
declare
  a varchar2(20);
  b number;
begin
  b := 10000 / round(dbms_random.value(-0.1, 1.1));
  select ename into a from emp where empno = 2222222222;
...
exception
   when others then
     dbms_output.put_line('womdyi 得到了有效处理');
end;
/
