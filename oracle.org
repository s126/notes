...
#+BEGIN_SRC sql
-- 语法
create table 表名 as select cause...
...
-- 共有同义词的使用。一次创建，每个用户都可以使用。
grant create public synonym to vip;
create public synonym e for sott.emp;
...
-- 自动插入，需要配合触发器 (triggle) 使用。如果有兴趣、有余力的同学，请自己关注。其他人无视则可。
...
表分区是为了达到分而治之的目的，是鸡蛋不能放在同一个篮子里的切实实践。首先能提高效率，再者能保证安全。
...
-- 复合分区中进行查询
select * from cxbe3 subpartition (p1_ps4);   -- 表示 p1 子分区下的 ps4 分区
...
#+BEGIN_SRC sql
-- 基本创建
create table bea1 (name varchar2(20), birth date, yz number);   -- 表的创建，不带分区的情况
...
[[http://www.bing.com/knows/search?q=%E5%93%88%E5%B8%8C%E7%AE%97%E6%B3%95&mkt=zh-cn&FORM=BKACAI][哈希算法]],散列表是也~
...
-- 指定表空间
create table beax (name varchar2(20), yz number)
partition by hash (name) partitions 5 stores in (users, system, users, system, system);
...
- 合并相邻分区
  : alter table bea2 merge partition p2, p3 into partition p8;
- 拆分分区
  : alter table bea2 split partition p2 at (3000) into (partition p3, partition p14);
  : -- 其他类型的
- 重命名分区
