...
*** 标量类型(Scalar)，基本等同于 sql 中的数据类型，比如 varchar2, number, date, blob, raw 等，还包括 boolean 类型。
...
Oracle 运行过程中出现的错误，称之为 Oracle 异常。主要分为两种，一种是系统自定义异常(即内部异常)，一种是用户自定义异常。
...
#+BEGIN_SRC sql
declare
  nested_delete exception;   -- 先自定定义异常，名字为 nested_delete
  PRAGMA EXCEPTION_INIT(nested_delete, -2292);  -- 以为记录存在外键约束这个系统异常只有一个编号为 -2292，没有名字。我们通过这个语句，给 -2292 这个异常赋予名字 nested_delete。
begin
  delete from dept where deptno = 10;           -- 这条记录存在外键约束，所以删除失败，抛出异常 -2292。因为上面定义了 -2292 为 nested_delete，所以会抛出一个 nested_delete 异常。
exception
...
: raise exception
如果想抛出一个带详细信息的异常，可以用到 Oracle 提供给我们的 raise_application_error，这是一个存储过程。
